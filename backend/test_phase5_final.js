...
async function runTests() {
  ...
  const sandboxConfig = new SandboxConfiguration({ dockerEnabled: false });
  const executor = new SandboxedScanExecutor({ sandboxConfig });
  const queueProvider = queueService.getQueue('scan', { concurrency: 5 });
  ...
  console.log(`\n🎉 Phase 5.4 Stress Test Passed: ${assertions} assertions completed.`);
}
